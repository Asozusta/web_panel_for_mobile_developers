...
class FileUploader {
	...
	$sql = "INSERT INTO ".$_SESSION['last_post_type']." (post_id,address) values(?,?)";
    	$q = $pdo->prepare($sql);
    	$q->execute(array($_SESSION['last_post_id'],$metas['name']));
    ...
    private function codeToMessage($code, $file = null) {
	    ...
	    switch ($code) {
	        case UPLOAD_ERR_INI_SIZE:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up346";
	            break;
	        case UPLOAD_ERR_FORM_SIZE:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up347";
	            break;
	        case UPLOAD_ERR_PARTIAL:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up348";
	            break;
	        case UPLOAD_ERR_NO_FILE:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up349";
	            break;
	        case UPLOAD_ERR_NO_TMP_DIR:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up350";
	            break;
	        case UPLOAD_ERR_CANT_WRITE:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up351";
	            break;
	        case UPLOAD_ERR_EXTENSION:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up352";
	            break;

	            case 'user_limit':
                $message = "شما به محدودیت حجم آپلود خود رسیده اید , از طریق بخش فروشگاه این محدودیت را کاهش دهید ";
	            break;

	            case 'no_confirm':
                $message = "برای آپلود فایل ابتدا حساب کاربری خود را فعال کنید";
	            break;

            case 'accepted_file_types':
                $message = "نوع فایل " . $file['old_name']." مورد قبول نیست ";
                break;
            case 'file_uploads':
                $message = "File uploading option in disabled in php.ini";
                break;
            case 'max_file_size':
                $message = "حجم فایل ". $file['old_name'] . " بسیار زیاد است ";
                break;
            case 'max_files_size':
                $message = "مجموع حجم فایل ها بسیار زیاد است";
                break;
            case 'max_number_of_files':
                $message = "تعداد فایل های انتخاب شده از حداکثر مجاز بیشتر است";
                break;
            case 'required_and_no_file':
                $message = "فایلی انتخاب نشده است";
                break;
		case 'invalid_folder_path':
			$message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up353";
		break;
	        default:
	            $message = "خطا را به مدیریت سایت اطلاع دهید - کد خطا : up354";
	            break;
	    }
