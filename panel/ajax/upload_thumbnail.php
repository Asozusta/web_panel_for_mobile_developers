 ...
 if (isset($_FILES['myfile']['name']))
{
    ...
    $saveto = "../../upload/thumbnails/".$address.".jpg";
    ...
    if ($typeok)
    {
        ...
        imagedestroy($src);
            ...
            $sql="SELECT thumbnail FROM posts WHERE id = ".clear($_SESSION['last_post_id']);
            foreach( $pdo->query($sql) as $row) { 
                $profilepicurl = $row['profilepicurl'];
            }
            if(isset($profilepicurl) && $profilepicurl!='')
            unlink('../../upload/thumbnails/'.$profilepicurl.'.jpg');



			$sql = "UPDATE posts  set thumbnail = ? WHERE id =".clear($_SESSION['last_post_id']);
			$q = $pdo->prepare($sql);
